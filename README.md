...
เพื่อให้คีออสสามารถสั่งพิมพ์บัตรคิวผ่านเครื่องพิมพ์ความร้อนได้โดยตรง โปรเจกต์มีบริการ Node.js แบบเบาๆ ที่อาศัยแพ็กเกจ [`node-thermal-printer`](https://www.npmjs.com/package/node-thermal-printer) สำหรับจัดการคำสั่งพิมพ์และเชื่อมต่อกับเครื่องพิมพ์ ESC/POS

### 1) ติดตั้งและเริ่มบริการพิมพ์

```bash
npm install        # ติดตั้ง dependencies รวมถึง node-thermal-printer
...
| `QUEUE_PRINTER_TYPE` | `epson` | ประเภทเครื่องพิมพ์ (`epson`, `star`, `tanca`, `daruma`, `brother`, `custom`) |
| `QUEUE_PRINTER_CHARSET` | `TIS11_THAI` | โค้ดเพจสำหรับตัวอักษร (รองรับภาษาไทย) |
...
ต้องการใช้ร่วมกับหน้า Kiosk ที่มีอยู่ เพียงเปิดใช้งานการพิมพ์แบบ BIXOLON แล้วตั้งค่า `bixolon_service_url` ให้ชี้มายังบริการนี้ (เช่น `http://127.0.0.1:18080`) ก็จะสามารถสั่งพิมพ์ผ่าน `node-thermal-printer` ได้ทันที หากตั้งค่า `bixolon_printer_interface` เป็น URI เต็มรูปแบบ (เช่น `tcp://192.168.0.50:9100` หรือ `printer:POS-80C`) บริการจะใช้ค่าดังกล่าวได้โดยตรงโดยไม่ต้องตั้งค่า Environment เพิ่มเติม
