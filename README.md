...
<<<<<<< HEAD
เพื่อให้คีออสสามารถสั่งพิมพ์บัตรคิวผ่านเครื่องพิมพ์ความร้อนได้โดยตรง โปรเจกต์มีบริการ Node.js แบบเบาๆ ที่ใช้ [`esc-pos-encoder`](https://www.npmjs.com/package/esc-pos-encoder) ในการสร้างคำสั่ง ESC/POS (รองรับภาษาไทย) แล้วส่งตรงไปยังเครื่องพิมพ์ผ่านซ็อกเก็ต TCP
=======
เพื่อให้คีออสสามารถสั่งพิมพ์บัตรคิวผ่านเครื่องพิมพ์ความร้อนได้โดยตรง โปรเจกต์มีบริการ Node.js แบบเบาๆ ที่ใช้ [`escpos`](https://www.npmjs.com/package/escpos) ร่วมกับ [`escpos-network`](https://www.npmjs.com/package/escpos-network) ในการสร้างและส่งคำสั่ง ESC/POS (รองรับภาษาไทย) ไปยังเครื่องพิมพ์ผ่านซ็อกเก็ต TCP
>>>>>>> origin/codex/implement-queue-ticket-printing-using-package-btavfk

### 1) ติดตั้งและเริ่มบริการพิมพ์

```bash
<<<<<<< HEAD
npm install        # ติดตั้ง dependencies รวมถึง esc-pos-encoder
=======
npm install        # ติดตั้ง dependencies รวมถึง escpos และ escpos-network
>>>>>>> origin/codex/implement-queue-ticket-printing-using-package-btavfk
npm run start:queue-printer
```

บริการจะรับคำสั่งที่ `http://0.0.0.0:18080` โดยค่าเริ่มต้น และจะติดต่อเครื่องพิมพ์ตามค่าที่กำหนดใน `QUEUE_PRINTER_INTERFACE` (เช่น `tcp://192.168.0.50:9100` หรือ `printer:POS-80C`). สามารถกำหนดค่าเพิ่มเติมผ่าน Environment variables สำคัญดังนี้:

| ตัวแปร | ค่าเริ่มต้น | คำอธิบาย |
| --- | --- | --- |
| `QUEUE_PRINTER_INTERFACE` | *(จำเป็นหากไม่ส่งจากไคลเอนต์)* | URI การเชื่อมต่อเครื่องพิมพ์ เช่น `tcp://192.168.0.50:9100` |
| `QUEUE_PRINTER_CHARSET` | `thai11` | โค้ดเพจ/Codepage สำหรับตัวอักษร (รองรับภาษาไทย) |
| `QUEUE_PRINTER_COLUMNS` | `48` (หรือ `42` หากตั้ง `QUEUE_PRINTER_TYPE=star`) | จำนวนตัวอักษรต่อบรรทัดขณะจัดรูปแบบบัตรคิว |
| `QUEUE_PRINTER_ALLOWED_ORIGIN` | `*` | กำหนด CORS สำหรับการเรียกจากเบราว์เซอร์ |
| `QUEUE_PRINTER_PORT` | `18080` | พอร์ตที่บริการรับคำสั่ง |

> ℹ️ `QUEUE_PRINTER_TYPE` ยังรองรับเพื่อความเข้ากันได้ย้อนหลัง หากตั้งค่าเป็น `star` ระบบจะเลือกจำนวนคอลัมน์เริ่มต้นเป็น 42 อักขระโดยอัตโนมัติ

### 2) รูปแบบคำสั่งพิมพ์ที่รองรับ

บริการมี 2 endpoint ให้เลือกใช้ตามการทำงานที่ต้องการ:

1. `POST /commands/print` – รองรับ payload แบบเดียวกับ BIXOLON Web Print (ส่งข้อมูล ESC/POS ที่เข้ารหัส base64/hex/ascii)
2. `POST /print-ticket` – ส่งข้อมูลบัตรคิวเป็น JSON แล้วให้บริการจัดรูปแบบและสร้างคำสั่งพิมพ์ให้โดยอัตโนมัติ

ตัวอย่างการเรียกพิมพ์บัตรคิวพร้อม QR code:

```bash
curl -X POST http://localhost:18080/print-ticket \
  ...
  -d '{
        ...
        "ticket": {
          ...
          "footer": "กรุณารอเรียกคิวจากเจ้าหน้าที่"
        }
      }'
```

<<<<<<< HEAD
ต้องการใช้ร่วมกับหน้า Kiosk ที่มีอยู่ เพียงเปิดใช้งานการพิมพ์แบบ BIXOLON แล้วตั้งค่า `bixolon_service_url` ให้ชี้มายังบริการนี้ (เช่น `http://127.0.0.1:18080`) ก็จะสามารถสั่งพิมพ์ผ่านบริการใหม่นี้ได้ทันที หากตั้งค่า `bixolon_printer_interface` เป็น URI เต็มรูปแบบ (เช่น `tcp://192.168.0.50:9100` หรือ `printer:POS-80C`) บริการจะใช้ค่าดังกล่าวได้โดยตรงโดยไม่ต้องตั้งค่า Environment เพิ่มเติม และการเรนเดอร์บัตรคิวจะใช้ codepage ภาษาไทยของ `esc-pos-encoder`
=======
ต้องการใช้ร่วมกับหน้า Kiosk ที่มีอยู่ เพียงเปิดใช้งานการพิมพ์แบบ BIXOLON แล้วตั้งค่า `bixolon_service_url` ให้ชี้มายังบริการนี้ (เช่น `http://127.0.0.1:18080`) ก็จะสามารถสั่งพิมพ์ผ่านบริการใหม่นี้ได้ทันที หากตั้งค่า `bixolon_printer_interface` เป็น URI เต็มรูปแบบ (เช่น `tcp://192.168.0.50:9100` หรือ `printer:POS-80C`) บริการจะใช้ค่าดังกล่าวได้โดยตรงโดยไม่ต้องตั้งค่า Environment เพิ่มเติม และการเรนเดอร์บัตรคิวจะใช้ codepage ภาษาไทยผ่าน `escpos`
>>>>>>> origin/codex/implement-queue-ticket-printing-using-package-btavfk
