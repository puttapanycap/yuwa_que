...
| `QUEUE_PRINTER_CODE_TABLE` | *(กำหนดเองได้)* | เลข Code Table (`ESC t n`) ที่จะสั่งไปยังเครื่องพิมพ์ เช่น `21` สำหรับ TIS-620 |
...
> ✅ หากกำหนด `QUEUE_PRINTER_CHARSET` เป็นภาษาไทย (เช่น `thai11` หรือ `tis620`) บริการจะส่งคำสั่ง `ESC t 21` เพื่อปรับ Code Table สำหรับภาษาไทยให้อัตโนมัติ แต่หากเครื่องพิมพ์ต้องใช้ค่าอื่นสามารถตั้ง `QUEUE_PRINTER_CODE_TABLE` กำกับเองได้
