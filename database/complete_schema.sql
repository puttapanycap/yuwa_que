...
CREATE TABLE queues (
    ...
    INDEX idx_queues_created_date (DATE(creation_time)),
    INDEX idx_queues_status_date (current_status, DATE(creation_time))
);
...
CREATE TABLE service_flow_history (
    ...
    INDEX idx_queue_history_date (DATE(timestamp))
);
...
CREATE TABLE audit_logs (
    ...
    INDEX idx_audit_logs_date (DATE(timestamp))
);
...
-- ตารางสำหรับจัดการ Auto Reset Schedule
...
-- ตารางบันทึกการ Reset อัตโนมัติ
...
CREATE TABLE notification_deliveries (
    ...
    channel ENUM('browser', 'email', 'telegram', 'sms') NOT NULL,
...
CREATE TABLE IF NOT EXISTS report_execution_log (
    ...
    FOREIGN KEY (template_id) REFERENCES report_templates(template_id),
    FOREIGN KEY (executed_by) REFERENCES staff_users(staff_id)
);
...
-- Add foreign key for queue_types.last_reset_by
ALTER TABLE queue_types ADD FOREIGN KEY (last_reset_by) REFERENCES staff_users(staff_id);

-- Add foreign key for service_points.voice_template_id
...
-- Add foreign key for service_points.queue_type_id
ALTER TABLE service_points ADD FOREIGN KEY (queue_type_id) REFERENCES queue_types(queue_type_id);
...
INSERT INTO service_points (point_name, point_description, position_key, display_order) VALUES
...
INSERT INTO notification_preferences (staff_id, notification_type, email_enabled, browser_enabled, telegram_enabled, sound_enabled)
...
-- ข้อมูลเริ่มต้นสำหรับการตั้งค่า API
...
CREATE INDEX IF NOT EXISTS idx_queues_created_date ON queues(DATE(creation_time));
CREATE INDEX IF NOT EXISTS idx_queues_status_date ON queues(current_status, DATE(creation_time));
CREATE INDEX IF NOT EXISTS idx_queue_history_date ON service_flow_history(DATE(timestamp));
